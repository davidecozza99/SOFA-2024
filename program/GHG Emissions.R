...
##### RE update with the latest database
...
scenathon<- read_csv(here("data", "240523_FullDataBase.csv")) %>% 
  ...
  mutate(N2O_crop = sum(calccropn2o)/1000) %>% 
  mutate(CH4_crop = sum(calccropch4)/1000) %>% 
  mutate(CO2_crop = sum(calccropco2)/1000) %>% 
  mutate(N2O_live = sum(calcliven2o)/1000) %>% 
  mutate(CH4_live = sum(calclivech4)/1000) %>% 
  mutate(CO2_def = sum(calcdeforco2)/1000) %>% 
  mutate(CO2_other = sum(calcotherlucco2)/1000) %>%
  mutate(calcpeatco2 = as.numeric(calcpeatco2)) %>%
  mutate(CO2_peat = sum(calcpeatco2/1000, na.rm = TRUE)) %>% 
  mutate(CO2_aband = sum(calcsequestaband)/1000) %>% 
  mutate(CO2_affor = sum(calcsequestaffor)/1000) %>% 
  mutate(CO2_bio = sum(ghgbiofuels)/1000) %>% 
...
gas_colors <- c("N2O_crop" = "#943F3F", "N2O_live" = "#FF0000",
                ...
                "CO2_other" ="#D5E59E","CO2_peat" = "black",
                "CO2_aband" = "#66BB6A",
                "CO2_affor" = "#1B5E20",
                "CO2_bio"= "grey"
)
...
for (current_pathway in unique_pathways) {
  ...
  p <- ggplot(df_long, aes(x = Year, y = Emission, fill = Gas)) +
    geom_area() +
    labs(title = paste("AFOLU GHG emissions"), x = "", y = "GtCO2eq", fill = "") +
   ...
   theme_minimal() +
    theme(
      ...
      legend.title = element_text(family = "sans", color = "steelblue", size = 18, face = "bold"),
      legend.text = element_text(family = "sans", size = 18),
      plot.title = element_text(color = "steelblue", size = 24, face = "bold"),
...
jpeg(here("output", "figures", paste0(gsub("-", "",Sys.Date()), "_", "GHG_SCENATHON.jpeg")),
...
#   
#   p <- ggplot(df_long, aes(x = Year, y = Emission, fill = Gas)) +
#     geom_bar(stat = "identity") +  # Change to geom_bar with stat = "identity"
...
#     theme_minimal() +
#     theme(
#       text = element_text(family = "Courier New", color = "black", size = 14, face = "bold"),
#       legend.title = element_text(family = "Courier New", color = "steelblue", size = 14, face = "bold"),
#       legend.text = element_text(family = "Courier New", size = 14),
...
# }
# 
# # Return or print one of the plots from the list
# p
# 
# 
