...
#Data extraction ---------------------------------------------------------------
scenathon<- read.csv(here("data", "240418_FullDataBase.csv"), sep = "") %>% 
  rename(alpha3 = country, Year = year) %>% 
  mutate(pathway = recode(pathway, "NationalCommitment" = "NationalCommitments")) %>% 
...
for (current_pathway in unique_pathways) {
  ...
  p <- ggplot(df_long, aes(x = Year, y = Emission, fill = Gas)) +
    ...
    scale_y_continuous(breaks = round(c(-7, -5, 2, 0, 5, 10, 15, 17))) +
...
tiff(here("output", "figures", paste0(gsub("-", "",Sys.Date()), "_", "GHG_SCENATHON.tiff")),
