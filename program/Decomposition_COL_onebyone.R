...
col_data <- read_xlsx(here("data", "report_COL_20240228_15H39.xlsx"), sheet = "Indicators") %>% 
...
for (element in elements) {
  ...
  current_plot <- col %>%
    ...
    geom_bar(stat = "identity", data = filter(col, !str_detect(Pathway, "complete") & !str_detect(Pathway, "tradeeffect")),
             aes(fill = scenarios)) +
