...
gbr_data <- read_xlsx(here("data", "report_GBR_20240228_16H58.xlsx"), sheet = "Indicators") %>% 
...
for (element in elements) {
  ...
  current_plot <- gbr %>%
    ...
    geom_bar(stat = "identity", data = filter(gbr, !str_detect(Pathway, "complete") & !str_detect(Pathway, "tradeeffect")),
             aes(fill = scenarios)) +
