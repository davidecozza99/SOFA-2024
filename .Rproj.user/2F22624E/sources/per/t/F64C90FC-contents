...
eth_data <- read_xlsx(here("data", "report_ETH_20240228_15H45.xlsx"), sheet = "Indicators") %>% 
...
figure_directory <- here("output", "decomposition", "ETH", paste0(gsub("-", "", Sys.Date())))
...
for (element in elements) {
  ...
  current_plot <- eth %>%
    ...
    geom_bar(stat = "identity", data = filter(eth, !str_detect(Pathway, "complete") & !str_detect(Pathway, "tradeeffect")),
             aes(fill = scenarios)) +
